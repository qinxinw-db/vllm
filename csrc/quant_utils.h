...
inline __device__ void load_int8_kv_cache_vec(Vec_k* tgt_vec, const T* cache_ptr, int cache_idx, float scale)
{
    ...
    // const auto quant = *reinterpret_cast<const Packed_Int8_t*>(&pointer[idx]);
...
template <typename Vec_k, typename T>
inline __device__ void store_int8_kv_cache_vec(T* cache, const Vec_k& src_vec, int cache_idx, float scale)
{
    // using Packed_Int8_t = typename packed_type<int8_t, num_elems<Vec_k>::value>::type;
    // using Packed_Float_t = typename packed_type<float, num_elems<Vec_k>::value>::type;
    // Packed_Int8_t out_quant = cast_to_int8(mul<Packed_Float_t>(scale, convert_to_float(vec)));
